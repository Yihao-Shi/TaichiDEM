...
class ContactPairs:
    def __init__(self, max_contact_num, partList, wallList, contModel):
        ...
        self.particleNeighbor = ti.field(int, shape=(partList.max_particle_num, 6))
        self.tangForceOld = ti.Vector.field(3, float, shape=(partList.max_particle_num, 6))
        self.Neighbor_num = ti.field(int, shape=partList.max_particle_num)
    ...
    def copyHistory(self, nc):
        end1 = self.endID1[nc]
        end2 = self.endID2[nc]
        count = ti.atomic_add(self.Neighbor_num[end1], 1)
        self.particleNeighbor[end1, count] = end2
        self.tangForceOld[end1, count] = self.ctforce[nc]
    ...
    def HistTangInfo(self, nc):
        ...
        end2 = self.endID2[nc]
        keyLoc = -1
        for loc in range(self.Neighbor_num[end1]):
            if self.particleNeighbor[end1, loc] == 0:
                break
            elif self.particleNeighbor[end1, loc] == end2:
                keyLoc = loc
                break
        return keyLoc
...
class Linear(ContactPairs):
    ...
    def ForceAssemblePP(self, nc, end1, end2, matID1, matID2, cpos):
        ...
        self.contModel.ComputeContactTangentialForce(self, nc, matID1, matID2, m_eff, v_rel, end1, keyLoc)
